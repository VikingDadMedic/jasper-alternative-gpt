    ...
    return (
        <div className="flex flex-wrap justify-start gap-2 my-6">
            { categoriesData.map( ( category: Category ) => (
                <button
                    key={ category.id }
                    className={ `cursor-pointer border inline-flex items-center justify-center mr-2 mb-2 px-3.5 py-1 text-sm font-medium rounded-full ${ selectedCategoryId === category.id
                        ? "border-rose-400 bg-rose-700 text-white hover:bg-opacity-100"
